...
import example.iotstar.config.DBConnectMySQL;
...
public class UserDaoImpl extends DBConnectMySQL implements IUserDao {
	...
	public void Update(UserModel user) {
		String query = "UPDATE users SET username=?, email=?, password=?, fullname=?, images=? WHERE id=?";
		try {
			conn = super.getDatabaseConnection();
			ps = conn.prepareStatement(query);
...
//		dm.Insert(newUserModel);
		...
		UserModel modelUpdate = new UserModel(1, "admin", "admin@gmail.com", "2357", "phamhanminhchuong",
				"https://images3.com");
		dm.Update(modelUpdate);
